...
/// Version MySQL — à utiliser avec Pomelo.EntityFrameworkCore.MySql.
///
/// Doit être appelé après l'exécution de seed_complet_mysql.sql.
/// Mot de passe universel : EduManager@2025!
///
/// Configuration EF Core requise dans Program.cs :
/// ─────────────────────────────────────────────────
...
///   "ConnectionStrings": {
///     "DefaultConnection":
///       "Server=localhost;Port=3306;Database=edu_manager;
///        User=root;Password=YOUR_PASSWORD;CharSet=utf8mb4;"
///   }
/// </summary>
public static class DbInitializerMySql
{
    // ── GUIDs partagés avec seed_complet_mysql.sql ────────────────────────────
    ...
    {
        // Applique les migrations en attente (crée le schéma si vide)
            ...
            if (!await roleManager.RoleExistsAsync(role))
            {
                ...
                if (!result.Succeeded)
                    throw new InvalidOperationException(
                        $"Impossible de créer le rôle '{role}': " +
